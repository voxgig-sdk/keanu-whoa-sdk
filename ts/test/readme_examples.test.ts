...
//     (\`new KeanuWhoaSDK(...)\` / \`KeanuWhoaSDK.test(...)\` become
//     \`KeanuWhoaSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"whoa":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new KeanuWhoaSDK(...)\` and any
// \`KeanuWhoaSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+KeanuWhoaSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/KeanuWhoaSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
