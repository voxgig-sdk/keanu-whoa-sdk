...
The Lua SDK for the KeanuWhoa API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Whoa(nil):list(nil, nil)
...
```lua
local result, err = client:Whoa(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:KeanuWhoa(nil):load(
  { id = "test01" }, nil
)
