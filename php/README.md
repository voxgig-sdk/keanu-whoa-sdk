...
The PHP SDK for the KeanuWhoa API. Provides an entity-oriented interface using PHP conventions.


## Install
```bash
composer require voxgig/keanu-whoa-sdk
```
...
```php
[$result, $err] = $client->Whoa(null)->list(null, null);
...
```php
[$result, $err] = $client->Whoa(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->KeanuWhoa(null)->load(
    ["id" => "test01"], null
);
