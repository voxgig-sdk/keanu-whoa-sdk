...
class KeanuWhoaMakeError
{
    public static function call(?KeanuWhoaContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new KeanuWhoaContext([], null);
        }
        $op = $ctx->op ?? new KeanuWhoaOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new KeanuWhoaResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof KeanuWhoaError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new KeanuWhoaError('', $msg, $ctx);
        ...
        if ($err instanceof KeanuWhoaError) {
