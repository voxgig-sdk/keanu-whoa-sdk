...
[MCP](https://modelcontextprotocol.io) server exposing the KeanuWhoa SDK as
two agent tools — `keanu-whoa_list` and `keanu-whoa_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/keanu-whoa-mcp)
make build

# 2. Provide credentials via the environment
export KEANU_WHOA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user keanu-whoa \
  -- /absolute/path/to/keanu-whoa-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./keanu-whoa-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// keanu-whoa_list: first page of records
{ "entity": "whoa" }
{ "entity": "whoa", "query": { } }

// keanu-whoa_load: one record by id
{ "entity": "whoa", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/keanu-whoa-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user keanu-whoa \
     -- "$PWD"/dist/*/keanu-whoa-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `keanu-whoa_list` and `keanu-whoa_load` tools now appear
   in new sessions. Ask the agent to *"list whoa using keanu-whoa"*
   and it calls `keanu-whoa_list` with `{"entity":"whoa"}`.
...
```sh
export KEANU_WHOA_APIKEY=sk_live_xxx            # API key
export KEANU_WHOA_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `keanu-whoa_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `keanu-whoa_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `KEANU_WHOA_APIKEY` | API key sent with every request. |
| `KEANU_WHOA_BASE` | Optional override of the API base URL. |
...
```sh
./keanu-whoa-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"keanu-whoa_load","arguments":{"entity":"whoa","query":{"id":1}}}}'
```
