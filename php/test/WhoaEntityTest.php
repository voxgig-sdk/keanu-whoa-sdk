...
class WhoaEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = KeanuWhoaSDK::test($seed, null);
        $seen = iterator_to_array($base->Whoa(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Whoa(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set KEANU_WHOA_TEST_WHOA_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $whoa_ref01_list_result = $whoa_ref01_ent->list($whoa_ref01_match, null);
        ...
        $whoa_ref01_data_dt0_loaded = $whoa_ref01_ent->load($whoa_ref01_match_dt0, null);
        $whoa_ref01_data_dt0_load_result = Helpers::to_map(is_object($whoa_ref01_data_dt0_loaded) && method_exists($whoa_ref01_data_dt0_loaded, 'data_get') ? $whoa_ref01_data_dt0_loaded->data_get() : $whoa_ref01_data_dt0_loaded);
...
function whoa_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("KEANU_WHOA_TEST_WHOA_ENTID");
