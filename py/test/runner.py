...
class KeanuWhoaTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            KeanuWhoaTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = KeanuWhoaTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = KeanuWhoaTestRunner.getenv("KEANU_WHOA_TEST_LIVE")
        override = KeanuWhoaTestRunner.getenv("KEANU_WHOA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = KeanuWhoaTestRunner.getenv(key)
        ...
        explain = KeanuWhoaTestRunner.getenv("KEANU_WHOA_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if KeanuWhoaTestRunner._test_control is not None:
            return KeanuWhoaTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                KeanuWhoaTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            KeanuWhoaTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = KeanuWhoaTestRunner.load_test_control()
...
def load_env_local():
    KeanuWhoaTestRunner.load_env_local()


def env_override(m):
    return KeanuWhoaTestRunner.env_override(m)


def entity_data(v):
    return KeanuWhoaTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return KeanuWhoaTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return KeanuWhoaTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return KeanuWhoaTestRunner.load_test_control()


def live_client_options():
    return KeanuWhoaTestRunner.live_client_options()


def live_delay_ms():
    return KeanuWhoaTestRunner.live_delay_ms()
