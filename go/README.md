# KeanuWhoa Golang SDK



The Golang SDK for the KeanuWhoa API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.

It exposes the API as capitalised, semantic **Entities** — e.g. `client.Whoa(nil)` — each with the same small set of operations (`List`, `Load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
## Install
```bash
go get github.com/voxgig-sdk/keanu-whoa-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/keanu-whoa-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/keanu-whoa-sdk/go=../keanu-whoa-sdk/go
```
...
func main() {
    ...
    // List whoa records — the value is the array of records itself.
    whoas, err := client.Whoa(nil).List(nil, nil)
    if err != nil {
        panic(err)
    }
    for _, item := range whoas.([]any) {
        fmt.Println(item)
    }

    // Load a single whoa — the value is the loaded record.
...
whoa, err := client.Whoa(nil).List(
    nil, nil,
)
if err != nil {
    panic(err)
}
fmt.Println(whoa) // the returned mock data
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *KeanuWhoaSDK
```
...
| `Whoa` | `(data map[string]any) KeanuWhoaEntity` | Create a Whoa entity instance. |

### Entity interface (KeanuWhoaEntity)

All entities implement the `KeanuWhoaEntity` interface.
...
slice):
    ...
    whoa, err := client.Whoa(nil).List(map[string]any{/* fields */}, nil)
    if err != nil { /* handle */ }
    // whoa is the returned record
...
Create an instance: `whoa := client.Whoa(nil)`
...
fmt.Println(whoa) // the loaded record
```
...
fmt.Println(whoas) // the array of records
```
...
```
github.com/voxgig-sdk/keanu-whoa-sdk/go/
├── keanu-whoa.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/keanu-whoa-sdk/go`) re-exports everything needed
