...
class WhoaEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new WhoaEntity($this->_client, $opts);
    }

    /**
     * @param Whoa|array $args Whoa data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = KeanuWhoaHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Whoa|array The current Whoa data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Whoa fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = KeanuWhoaHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Whoa fields).
     */
     ...
     * Load a single Whoa.
     *
     * @param WhoaLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed WhoaLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Whoa|array The loaded Whoa as an assoc-array at the
     *   SDK boundary; throws KeanuWhoaError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = KeanuWhoaHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param WhoaListMatch|array|null $reqmatch Match filter (any subset
     *   of Whoa fields) as an assoc-array; WhoaListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Whoa[]|array A list of Whoa items as assoc-arrays at
     *   the SDK boundary; throws KeanuWhoaError on failure (item-5 convention).
     */
