...
The Python SDK for the KeanuWhoa API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Whoa(None).list(None, None)
if err:
    raise Exception(err)
...
```python
result, err = client.Whoa(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.KeanuWhoa(None).load(
    {"id": "test01"}, None
)
