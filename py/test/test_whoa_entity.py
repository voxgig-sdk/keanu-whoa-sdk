...
class TestWhoaEntity:
    ...
    def test_should_stream(self):
        ...
        base = KeanuWhoaSDK.test(seed, None)
        seen = list(base.Whoa(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = KeanuWhoaSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Whoa(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set KEANU_WHOA_TEST_WHOA_ENTID JSON to run live")
        client = setup["client"]
        ...
        whoa_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(whoa_ref01_data_dt0_loaded))
...
def _whoa_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("KEANU_WHOA_TEST_WHOA_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("KEANU_WHOA_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("KEANU_WHOA_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("KEANU_WHOA_TEST_EXPLAIN") == "TRUE",
        "live": _live,
