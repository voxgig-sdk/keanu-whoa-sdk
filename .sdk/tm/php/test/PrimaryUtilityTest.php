...
// KeanuWhoa SDK primary utility test
//
...
//   KeanuWhoaOmni::unwrap, which throws the err so omni can match it
...
//   builds the typed context with KeanuWhoaOmni::ctx_from_map, runs the
//   utility, and writes the observable state back with
//   KeanuWhoaOmni::sync_ctx BEFORE throwing any error - that map is
...
class PrimaryUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = KeanuWhoaOmni::makeRunner(
                self::TEST_JSON_FILE, KeanuWhoaSDK::test(null, null));
    ...
    private static function client(): KeanuWhoaSDK
    {
    ...
    public function test_done_basic(): void
    {
        ...
        $this->runsection('done', function (array $ctxmap) use ($client, $utility) {
            $ctx = KeanuWhoaOmni::ctx_from_map($ctxmap, $client, $utility);
    ...
    public function test_make_error_basic(): void
    {
        ...
        $this->runsection('makeError', function ($ctxmap = null, $errmap = null) use ($client, $utility) {
            $ctx = KeanuWhoaOmni::ctx_from_map(
                is_array($ctxmap) ? $ctxmap : [], $client, $utility);
            $err = KeanuWhoaOmni::err_from_map(is_array($errmap) ? $errmap : null);
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new KeanuWhoaResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new KeanuWhoaBaseFeature();
    ...
    {
        $hook_client = KeanuWhoaSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends KeanuWhoaBaseFeature {
            public $hook_fn = null;
            public function TestHook(KeanuWhoaContext $ctx): void
            {
    ...
    {
        $init_client = KeanuWhoaSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends KeanuWhoaBaseFeature {
            public $init_fn = null;
    ...
    {
        $blocked_client = new KeanuWhoaSDK([
            'base' => 'http://localhost:8080',
            'system' => [
    ...
    public function test_make_request_basic(): void
    {
        ...
        $this->runsection('makeRequest', function (array $ctxmap) use ($client, $utility) {
            ...
            KeanuWhoaOmni::sync_ctx($ctxmap, $ctx);
            if ($err !== null) {
                throw $err;
            }
            return null;
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new KeanuWhoaSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = KeanuWhoaSDK::test(null, is_array($setup) ? $setup : null);
        $spec_utility = $spec_client->get_utility();
        $utility = self::client()->get_utility();

        $this->runsection('makeSpec', function (array $ctxmap) use ($spec_client, $spec_utility, $utility) {
            $ctx = KeanuWhoaOmni::ctx_from_map($ctxmap, $spec_client, $spec_utility);
    ...
    public function test_make_url_basic(): void
    {
        ...
        $this->runsection('makeUrl', function (array $ctxmap) use ($client, $utility) {
            ...
            return KeanuWhoaOmni::unwrap(($utility->make_url)($ctx));
        });
    }
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = KeanuWhoaSDK::test(null, is_array($setup) ? $setup : null);
        ...
        $this->runsection('prepareAuth', function (array $ctxmap) use ($auth_client, $auth_utility, $utility) {
            $ctx = KeanuWhoaOmni::ctx_from_map($ctxmap, $auth_client, $auth_utility);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runsection('resultBasic', function (array $ctxmap) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof KeanuWhoaError)
