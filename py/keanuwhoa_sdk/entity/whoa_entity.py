...
from keanuwhoa_sdk.keanuwhoa_types import (
    Whoa,
    WhoaLoadMatch,
    WhoaListMatch,
)
...
class WhoaEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return WhoaEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Whoa:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Whoa().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Whoa]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Whoa().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
