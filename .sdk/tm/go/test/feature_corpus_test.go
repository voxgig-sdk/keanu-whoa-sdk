...
//
// NewKeanuWhoaSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.KeanuWhoaSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewKeanuWhoaSDK(opts)
}
...
func fcCandidates(client *sdk.KeanuWhoaSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.KeanuWhoaSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.KeanuWhoaSDK, name string) any {
