...
class FtCtrl extends KeanuWhoaControl
{
...
class FtClient
{
    ...
    private KeanuWhoaUtility $utility;

    public function __construct(KeanuWhoaUtility $utility, array $options)
    {
    ...
    public function get_utility(): KeanuWhoaUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (KeanuWhoaContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public KeanuWhoaContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (KeanuWhoaContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = KeanuWhoaConfig::make_feature($name);
    ...
    private static function buildUrl(KeanuWhoaSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof KeanuWhoaError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new KeanuWhoaResponse($fetched);
        }
    ...
    private function fail(KeanuWhoaContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (KeanuWhoaContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
