...
import (
	...
	omni "github.com/voxgig-sdk/keanu-whoa-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.KeanuWhoaError); is {
				out := map[string]any{
					"name":    "KeanuWhoaError",
...
func omniCtx(arg any, client *sdk.KeanuWhoaSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.KeanuWhoaSDK); is {
			client = live
