...
class KeanuWhoaSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = KeanuWhoaHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = KeanuWhoaHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!KeanuWhoaFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, KeanuWhoaFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return KeanuWhoaUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = KeanuWhoaHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = KeanuWhoaHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = KeanuWhoaHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new KeanuWhoaError($op . "_allow",
                "KeanuWhoaSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = KeanuWhoaHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new KeanuWhoaError("graphql_error",
                "KeanuWhoaSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Whoa($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_whoa === null) {
                $this->_whoa = new WhoaEntity($this, null);
            }
            return $this->_whoa;
        }
        return new WhoaEntity($this, $data);
    }
