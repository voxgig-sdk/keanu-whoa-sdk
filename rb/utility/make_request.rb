...
module KeanuWhoaUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = KeanuWhoaResponse.new({})
    ...
    if fetch_err
      ...
      response = KeanuWhoaResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = KeanuWhoaResponse.new(fetched)
    else
