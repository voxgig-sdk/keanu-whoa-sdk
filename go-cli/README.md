...
boru-driven command-line client **and** interactive REPL for the KeanuWhoa
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/keanu-whoa-cli)
make build
...
./keanu-whoa-cli list whoa
./keanu-whoa-cli load 1 whoa            # {id:1} shorthand
./keanu-whoa-cli load '{id:1}' whoa       # explicit match map

# 5. Override the API base URL for a single call
KEANU_WHOA_BASE=https://api.example.com ./keanu-whoa-cli list whoa

# 6. No arguments -> interactive REPL
./keanu-whoa-cli
keanu-whoa> list whoa
keanu-whoa> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/keanu-whoa-cli
   ```
...
```sh
./keanu-whoa-cli load 1 whoa          # scalar shorthand for {id:1}
./keanu-whoa-cli load '{id:1}' whoa     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `keanu-whoa>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `whoa`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/keanu-whoa-cli`. |
...
(`./keanu-whoa-cli <expr>`) and interactively (the REPL), and expressions compose the
