...
MCP server exposing the KeanuWhoa SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `keanu-whoa_list` | `entity`, optional `query` map | First page of records as JSON |
| `keanu-whoa_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `keanu-whoa_list` and
`keanu-whoa_load` tools become available in new sessions.
