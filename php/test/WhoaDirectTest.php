...
class WhoaDirectTest extends TestCase
{
    public function test_direct_list_whoa(): void
    {
        $setup = whoa_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-whoa", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
    ...
    public function test_direct_load_whoa(): void
    {
        $setup = whoa_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-whoa", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
...
function whoa_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new KeanuWhoaSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
